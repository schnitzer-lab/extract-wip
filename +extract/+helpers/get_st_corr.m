function [st_corrs, t_maxes_in_active_regions] = get_st_corr(M, im, trace, ...
    ...
    % Get x-y ranges
    [x_range, y_range] = extract.helpers.get_image_xy_ranges(im> im_threshold, im_extension);
    ...
    % Get active periods
    active_frames = extract.helpers.get_active_frames(trace > t_treshold * max(trace), 2);
