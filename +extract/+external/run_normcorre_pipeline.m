function run_normcorre_pipeline(input, output, config)
    ...
    switch file_type
        case 'h5'
            [input_filename, input_datasetname] = extract.internal.parse_movie_name(input);
    ...
    [output_filename, output_datasetname] = extract.internal.parse_movie_name(output);
    ...
    if isempty(template)
        ...
        switch file_type
            ...
            case 'tif'
                im1 = single(extract.external.read_from_tif(input, 1, nt_template));
            case 'tiff'
                im1 = single(extract.external.read_from_tif(input, 1, nt_template));
        end

        if bandpass
            im1 = extract.internal.spatial_bandpass(im1, avg_cell_radius, 10, 2, use_gpu);
        end
    ...
    for i = 1:numel(startno)
        ...
        switch file_type
            ...
            case 'tif'
                M = single(extract.external.read_from_tif(input, startno(i), perframes(i)));
            case 'tiff'
                M = single(extract.external.read_from_tif(input, startno(i), perframes(i)));
        end

        if bandpass
            M_proc = extract.internal.spatial_bandpass(M, avg_cell_radius, 10, 2, use_gpu);
        else
            M_proc = M;
        end
        ...
        if nonrigid_mc
            ...
            if bandpass
                M_proc = extract.internal.spatial_bandpass(M_rigid, avg_cell_radius, 10, 2, use_gpu);
            else
                M_proc = M_rigid;
            end
