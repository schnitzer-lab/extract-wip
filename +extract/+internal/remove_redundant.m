function [x, is_bad] = remove_redundant( ...
    ...
    [fmap, ~] = extract.internal.get_quality_metric_map;
    ...
    function [idx_elim, is_attr_bad, merge] = ...
        ...
        % Tiny images
        is_S_tiny = (metrics(fmap('S_area_1'), :) <= thresholds.size_lower_limit) & ...
        (metrics(fmap('S_smooth_area_1'), :) <= thresholds.size_lower_limit);
        % Too large images
    ...
    function [merge, is_excess] = find_excess_components(M, corr_threshold, A2)
        ...
        if exist('A2', 'var')
            C = extract.internal.find_conncomp(M, corr_threshold, A2);
        else
            C = extract.internal.find_conncomp(M, corr_threshold);
        end
