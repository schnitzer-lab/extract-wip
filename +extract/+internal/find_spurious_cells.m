function [corr_score, scores_1, scores_2, scores_3] = ...
    ...
    if isempty(S_surround) || isempty(pre_T_in) || isempty(pre_T_out)
        ...
        pre_S_corr = S * 0;
        T_noise_limit = sqrt(2) * 3 * extract.internal.get_trace_noise(T);
        T_zm = max(0, bsxfun(@minus, T, T_noise_limit));
        T_zm = extract.internal.maybe_gpu(use_gpu, T_zm);

        for i = 1:num_chunks
            indices = extract.internal.select_indices(n, num_chunks, i);
            M_small = extract.internal.maybe_gpu(use_gpu, M(:, indices));
        ...
        % Process S_corr
        S_smooth = extract.internal.smooth_images(S, fov_size, GAUSS_FILTER_RADIUS_SMALL, use_gpu);
        ...
        % Process S_surround
        S_smooth = extract.internal.smooth_images(S, fov_size, GAUSS_FILTER_RADIUS_LARGE, use_gpu);
        S_smooth = extract.internal.normalize_to_one(S_smooth);
        Mask_surround = S_smooth > S_MASK_THRESHOLD;
        Mask_surround = single(Mask_surround);
        ...
        S_surround(Mask > 0) = 0;
        S_surround = extract.internal.normalize_to_one(S_surround);
        ...
        for i = 1:num_chunks
            ...
            pre_T_in(:, indices) = gather(extract.internal.maybe_gpu(use_gpu, S)' * M_small);
            pre_T_out(:, indices) = gather(extract.internal.maybe_gpu(use_gpu, S_surround)' * M_small);
            clear M_small;
        end
    ...
    for i = 1:size(T, 1)
        ...
        [active_frames, num_active_frames] = extract.internal.get_active_frames(t_mask > 0, 0);
        s1 = [];
        s2 = [];
        s3 = [];
