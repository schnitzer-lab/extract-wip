function M = correct_baseline(M, tau, remove_background, use_gpu)
    ...
    for i = 1:num_chunks
        indices = extract.internal.select_indices(m, num_chunks, i);
    ...
    function M = remove_baseline_func(M, tau, ppp, use_gpu)
        ...
        M = extract.internal.maybe_gpu(use_gpu, M);
        ...
        baselines = extract.internal.maybe_gpu(use_gpu, baselines);
        ...
        for idx_sample = 1:num_samples
            ...
            %         hist_counts = extract.internal.maybe_gpu(use_gpu, ...
        ...
        filt_out = extract.internal.maybe_gpu(use_gpu, filt_out);
        ...
        subt = extract.internal.maybe_gpu(use_gpu, subt);
