function [T_corr_in, T_corr_out, S_surround, S_smooth] = get_pre_correlations(S, T, M, avg_radius)
    ...
    mask = extract.internal.make_mask(single(S_smooth > 0.1), fov_size, round(avg_radius));
    ...
    for i_x = 1:np_x
        idx_x = extract.internal.select_indices(w, np_x, i_x);

        for i_y = 1:np_y
            idx_y = extract.internal.select_indices(h, np_y, i_y);
    ...
    [S_out, scale_s] = extract.internal.normalize_to_one(S_out);
