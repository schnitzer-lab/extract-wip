function cell_check(output, M)
    ...
    face_alpha = 0.1;
    default_colormap = flipud(extract.internal.brewermap.brewermap(64, 'rdgy'));
    h_neighbor_outlines = [];
    ...
    [is_attr_bad, metrics, is_elim] = extract.internal.get_cellcheck_features(output);
    ...
    [guess_good, guess_bad] = extract.internal.guess_labels_from_metrics(metrics);
    ...
    function plot_event_snapshots_with_neighbors
        ...
        % Get active periods
        active_frames = extract.internal.guess_labels_from_metrics(trace > 0.2 * max(trace), 2);
        % Transpose for convenience
        active_frames = active_frames';

        % Detect events and display N of them
        event_frames = extract.internal.detect_ca_events(trace, 0.5);
        ...
        [x_lims, y_lims] = extract.internal.get_image_xy_ranges(im > 0.2, 5);
        ...
        if isempty(handle_snapshots{1})
            ...
            axis(ax, 'image', 'off');
            colormap(ax, flipud(extract.internal.breweramp.brewermap(64, 'rdbu')));
        else
    ...
    function [x_current, y_current] = get_active_region(idx_current_cell)
        ...
        [x_range, y_range] = extract.internal.get_image_xy_ranges(images > 0.2, 1);
    ...
    %         for i = 1:size(ims, 3)
    %             im = ims(:, :, i);
    %             [x_range, y_range] = extract.internal.get_image_xy_ranges(im, 5);
    ...
    %         [x_range, y_range] = extract.internal.get_image_xy_ranges(im, 0);
    ...
    function [output, output_handle] = parse_output(output)
        ...
        if ischar(output)
            ...
            if ischar(M)
                ...
                [path, ~] = extract.internal.parse_movie_name(M);
